...
class SubnetScanner:
    ...
    def parse_network_intelligently(self, subnet: str) -> List[ipaddress.IPv4Network]:
        """
        Break down large networks into manageable chunks
        Prioritize commonly used subnets
        """
        ...
        logger.info(f"[SUBNET-SCAN] Analyzing network {network} with {network_size} addresses (/{prefix_len})")
        ...
        if prefix_len >= 24:
            ...
            # For /16, intelligently select subnets
            if prefix_len == 16:
                # Common patterns for /16 networks (e.g., 10.0.0.0/16)
                ...
                logger.info(f"[SUBNET-SCAN] Added {len(subnets_to_scan)} priority /24 subnets for /16 network")
                
                # Optionally add more subnets based on configuration
                # You could scan all 256 /24 subnets, but that would take time
                
            else:
                # For /17-/19, split into /24s but limit the number
                all_subnets = list(network.subnets(new_prefix=24))
                
                # Take first N subnets and some distributed samples
                ...
                logger.info(f"[SUBNET-SCAN] Selected {len(subnets_to_scan)} /24 subnets from {len(all_subnets)} total")
                
        else:  # Smaller than /16 (larger network)
            # For huge networks, just scan specific subnets
            logger.warning(f"[SUBNET-SCAN] Very large network (/{prefix_len}). Scanning limited subnets only.")
            
            # Just scan the first few /24 subnets
            all_subnets = list(network.subnets(new_prefix=24))[:10]
            subnets_to_scan = all_subnets
    ...
    async def scan_large_network(self, subnet: str, scan_id: str, scan_tracker=None, 
        ...
        try:
            # Parse network into subnets
            subnets = self.parse_network_intelligently(subnet)
            ...
            # First pass: Quick probe to find active subnets
            active_subnets = []
            
            if scan_mode == 'smart':
                logger.info("[SUBNET-SCAN] Performing quick probe to find active subnets...")
                ...
                # Scan first half of subnets plus any that look interesting
                active_subnets = subnets[:len(subnets)//2] if len(subnets) > 10 else subnets
    ...
    async def scan_subnet(self, subnet: ipaddress.IPv4Network, scan_id: str) -> Dict:
        ...
        # Use the advanced scanner if available
        if self.advanced_scanner:
            # Convert subnet to string and scan
            from .advanced_scanner import advanced_network_scan
            subnet_devices, _ = await asyncio.to_thread(
            ...
            hosts = list(subnet.hosts())[:256]  # Limit to 256 per subnet
            
            with ThreadPoolExecutor(max_workers=50) as executor:
                futures = {executor.submit(self.scan_host, str(ip)): ip for ip in hosts}
